...
export default function OnTvTVShowsPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const tvShowsData = await fetchOnTheAirTVShows(currentPage);
        setData(tvShowsData);
      } catch (error) {
  ...
  return (
    <div className={styles.pageContainer}>
      <div className={styles.contentWrapper}>
        <h1 className={styles.pageTitle}>ON TV</h1>
        
        {loading ? (
          <div className={styles.loadingContainer}>
            <Loader2 className={styles.loadingSpinner} size={40} />
            <p>Loading on-tv shows...</p>
          </div>
        ) : !data ? (
          <div className={styles.errorContainer}>
            ...
            <button 
              ...
              onClick={() => fetchOnTheAirTVShows(currentPage).then(setData)}
            >
              Retry
            </button>
          </div>
